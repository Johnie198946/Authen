...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "requested_entitlements", postgresql.JSONB(astext_type=sa.Text()),
    ...
    op.create_index(
        ...
        "organization_subscription_requests", ["organization_id", "application_id"],
        unique=True, postgresql_where=sa.text("status = 'pending'"),
    )


def downgrade() -> None:
    op.drop_index("uq_org_app_pending_subscription_request", table_name="organization_subscription_requests")
